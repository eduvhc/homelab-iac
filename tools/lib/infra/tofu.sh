...
IP_LIDARR=$(   _get lxc_ips .lidarr)
IP_YTDL_SUB=$( _get lxc_ips '["ytdl-sub"]')
ALL_LXC_IPS="$IP_ADGUARD $IP_GATEWAY $IP_COOLIFY $IP_RUNNER $IP_NAVIDROME $IP_LIDARR $IP_YTDL_SUB"
...
_val=
for _var in IP_ADGUARD IP_GATEWAY IP_COOLIFY IP_RUNNER IP_NAVIDROME IP_LIDARR IP_YTDL_SUB LAN_CIDR LAN_GATEWAY COOLIFY_API_URL; do
  eval "_val=\$$_var"
  case $_val in
    ''|null)
...
export IP_ADGUARD IP_GATEWAY IP_COOLIFY IP_RUNNER IP_NAVIDROME IP_LIDARR IP_YTDL_SUB ALL_LXC_IPS
