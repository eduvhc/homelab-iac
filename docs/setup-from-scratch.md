...
| Authelia OIDC client | `services/gateway/authelia/configuration.yml.tmpl` | `services/gateway/authelia/ops/sync.sh` (hybrid: hash + engine) |
...
| Add / rotate a secret | `sops iac/secrets.sops.yaml` (or `FORCE=1 services/coolify/ops/rotate-token.sh` for Coolify) | commit + push the encrypted file |
...
services/coolify/ops/rotate-token.sh` to mint a fresh token and write it to
